...
import yt.szczurek.selenium.eventapi.SeleniumMod;
...
public class SeleniumEventAPI {
    ...
    public void registerEventListeners(Object listener) {
        ...
        for (Method listenerMethod: methods) {
            ...
            if (listenerMethod.getParameterTypes().length != 1) {
                SeleniumMod.LOGGER.warn("Attempted to register an event that has more or less then one arg. Method " + listenerMethod.getName() + " in class " + listenerMethod.getDeclaringClass().getSimpleName());
            }
            // Make sure that one arg is an event.
            final Class<?> clazz;
            if (! Event.class.isAssignableFrom(clazz = listenerMethod.getParameterTypes()[0])) {
                SeleniumMod.LOGGER.warn(clazz.getSimpleName() + " is not an event!");
            }
